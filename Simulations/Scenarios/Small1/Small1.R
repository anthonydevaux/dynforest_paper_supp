...
evt.link <- list(model = ~ I(marker1_bi1 + beta1_1) + I(marker2_bi0 + beta0_2) +
                   I(marker1_bi1 + beta1_1)*I(marker2_bi0 + beta0_2),
                 gamma = c(3, -2, -3.5))
...
for (replicate in replicates.min:replicates.max){
  ...
  save(res, testData, trainData, file = "Scenarios/Small1/Small1_resu.RData")
