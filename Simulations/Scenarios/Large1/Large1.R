...
evt.link <- list(model = ~ I(marker1_bi0 + beta0_1) +
                   I(marker2_bi1 + beta1_2) +
                   I(marker3_bi1 + beta1_3) +
                   I(marker5_bi1 + beta1_5) +
                   # interaction
                   I(marker1_bi0 + beta0_1)*I(marker2_bi1 + beta1_2) +
                   I(marker3_bi1 + beta1_3)*I(marker5_bi1 + beta1_5)
...
for (replicate in replicates.min:replicates.max){
  ...
  if (length(res)==0){
    ...
    save(OOB.err_Dyn, best.mtry_Dyn, file = "Scenarios/Large1/Large1_mtry_DynForest.RData")
  }
  ...
  if (length(res)==0){
    ...
    save(OOB.err_2steps, best.mtry_2steps, file = "Scenarios/Large1/Large1_mtry_2steps.RData")
  ...
  res[[replicate]] <- list(oob.err = list(res_dyn$oob.err, res_2step_dyn$oob.err),
                           Var_depth = list(dyn_depth, dyn_2step_depth),
                           VIMP = list(vimp.Dyn, vimp.2steps),
                           pred = list(pred_dyn,
                                       pred_2step_dyn,
                                       pred_KM),
  ...
  save(res, best.mtry, testData, trainData, file = "Scenarios/Large1/Large1_resu.RData")
